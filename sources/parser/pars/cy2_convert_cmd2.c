...
int cy2_convert_cmd1a(t_cmdconvert *c)
{
    ...
    while (1)
    {
        ...
        {
            // Ignorer explicitement le token pipe
                ...
                c->current_input->input[0] == '|')
            {
                ...
                while (c->current_input && c->current_input->type == 1)
                    c->current_input = c->current_input->next;
            }
            ...
            // CORRECTION: Synchroniser head_input avec current_input
            // Essentiel pour que find_delim et append_cmd fonctionnent correctement
            c->head_input = c->current_input;
            
            // Continuer la boucle pour traiter naturellement la commande suivante
            continue;
        }
    }
    return (1);
